				...
				{#if current[idxw]}
					{#each { length: 7 } as d, idxd (idxd)}
						{#if current[idxw][idxd] != 0}
							...
							<span
								class="date"
								class:selected={isSelected(current[idxw][idxd], month, endDate)}
								class:in-range={isInRange(current[idxw][idxd], month, hoveringEndDate)}
								class:in-range-right={isInRange(current[idxw][idxd], month, hoveringEndDate) &&
									idxd === 6}
								class:in-range-left={isInRange(current[idxw][idxd], month, hoveringEndDate) &&
									idxd === 0}
								on:keydown
								on:click={() => selectRange(current[idxw][idxd], month)}
								on:mouseenter={() => {
									dateMouseIn(current[idxw][idxd], month);
								}}
						...
						{:else if idxw < 1}
							<span
								...
								class:in-range={isInRange(prev[prev.length - 1][idxd], month - 1, hoveringEndDate)}
								...
								on:mouseout={dateMouseOut}
								on:blur>{prev[prev.length - 1][idxd]}</span
							>
						{:else}
							<span
								class="date other"
								class:selected={isSelected(next[0][idxd], month + 1, endDate)}
								class:in-range={isInRange(next[0][idxd], month + 1, hoveringEndDate)}
								class:in-range-right={isInRange(next[0][idxd], month + 1, hoveringEndDate) &&
									idxd === 6}
								class:in-range-left={isInRange(next[0][idxd], month + 1, hoveringEndDate) &&
									idxd === 0}
								on:keydown
								on:click={() => selectRange(next[0][idxd], month + 1)}
								on:mouseenter={() => {
									dateMouseIn(next[0][idxd], month + 1);
								}}
								on:mouseout={dateMouseOut}
								on:blur>{next[0][idxd]}</span
							>
						{/if}
					{/each}
				{/if}
...
<style type="postcss">
	...
	.wrapper {
		@apply border border-[#EAF0ED] rounded-2xl w-[273px];
